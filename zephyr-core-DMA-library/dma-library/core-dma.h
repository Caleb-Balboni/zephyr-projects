...
typedef int (*sync_receive_t)(const struct device* dev, void* data, size_t data_size, struct k_timeout_t wait_time);
...
static inline int dma_core_sync_receive(const struct device* dev, void* data, size_t data_size, struct k_timeout_t wait_time) {
	const struct dma_engine* engine_api = (struct dma_engine*)dev->api;
	return engine_api->sync_receive(dev, data, receive_size, wait_time);
}

static inline int dma_core_async_receive(const struct device* dev, void (*callback_func)(void*, void*), void* user_data) {
	const struct dma_engine* engine_api = (struct dma_engine*)dev->api;
