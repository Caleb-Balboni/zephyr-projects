...
int fs_cmd_ls(const char* file_path) {
	struct fs_dir_t zdp;
	fs_dir_t_init(&zdp);
	if (!file_path) {
		int code;
...
int fs_cmd_mkdir(const char *file_path) {
	if (!file_path) {
...
int fs_cmd_rm(const char *file_path) {
	if (!file_path) {
...
int fs_cmd_cd(const char* file_path) {
	if (!file_path) {
...
int fs_cmd_touch(const char* file_path, size_t write_size, void* body) {
	if (!file_path) {
...
int fs_cmd_read(const char* file_path, size_t buf_size, void* buf) {
	if (!file_path) {
